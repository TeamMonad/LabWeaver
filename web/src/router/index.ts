...
const routes: RouteRecordRaw[] = [
  ...
  },
  {
    ...
    children: [
      { path: 'overview', component: () => import('@/views/teacher/TeacherOverviewView.vue') },
      { path: 'labs', component: () => import('@/views/teacher/LabListView.vue') },
      { path: 'environments', component: () => import('@/views/teacher/WorkbenchModuleView.vue'), props: { title: '环境', description: '在此查看实验运行环境、配额和健康诊断。' } },
      { path: 'evaluations', component: () => import('@/views/teacher/WorkbenchModuleView.vue'), props: { title: '评测', description: '在此查看评测队列、结果与需要教师处理的事项。' } },
      { path: 'resources', component: () => import('@/views/teacher/WorkbenchModuleView.vue'), props: { title: '资源', description: '在此管理实验材料、共享资源和关联版本。' } },
