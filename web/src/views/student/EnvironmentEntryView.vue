...
import type { EnvironmentInstanceSchema, EnvironmentTemplateReleaseSchema } from '@/generated/contracts'
...
const pendingRelease = ref<EnvironmentTemplateReleaseSchema | null>(null)
...
const releaseColumns: DataTableColumn<EnvironmentTemplateReleaseSchema & { actions?: never }>[] = [
...
async function createFromRelease(release: EnvironmentTemplateReleaseSchema) {
  ...
  try {
    ...
    if (!result.ok) {
      ...
      // Best-effort: if the backend ever returns a Location header or operation body with id, use it.
      // For now, leave the input for the user to fill from external source.
    }
  } finally {
