...
use crate::validation::{EvaluationSpecError, validate_spec};
...
/// Returns the generated JSON Schema for `EvaluationSpec` v1alpha1.
///
/// # Errors
///
...
pub fn goal_review_schema() -> Result<serde_json::Value, serde_json::Error> {
    serde_json::to_value(schema_for!(crate::GoalReview))
}

#[derive(Clone, Copy, Debug, Deserialize, JsonSchema, PartialEq, Serialize)]
enum EvaluationApiVersion {
    #[serde(rename = "evaluation.labweaver.io/v1alpha1")]
    V1Alpha1,
}
        ...
        match self {
            ...
            Self::Program {
                ...
                if test_groups.iter().any(|group| {
                    ...
                    return Err(EvaluationSpecError::InvalidStepConfiguration {
                        step_id: step_id.to_owned(),
                        detail: "test groups require a name, source, and non-zero weight"
                            .to_owned(),
                    });
                }
            ...
            Self::AnsibleProbe {
                ...
                if !read_only {
                    return Err(EvaluationSpecError::InvalidStepConfiguration {
                        step_id: step_id.to_owned(),
                        detail: "v1alpha1 ansible_probe must be read-only".to_owned(),
                    });
                }
                if module_allowlist
                    .iter()
                    .any(|module| !is_allowed_probe_module(module))
                {
                    return Err(EvaluationSpecError::InvalidStepConfiguration {
                        step_id: step_id.to_owned(),
                        detail: "ansible_probe contains a module outside the v1alpha1 allowlist"
                            .to_owned(),
                    });
                }
                Ok(())
...
impl TestGroup {
    ...
    /// Returns the relative group weight.
    #[must_use]
...
/// Deterministic Checker configurations frozen for v1alpha1.
#[derive(Clone, Debug, Deserialize, JsonSchema, PartialEq, Serialize)]
#[serde(tag = "kind", rename_all = "snake_case", deny_unknown_fields)]
...
impl ReviewPolicy {
    ...
    ///
    /// This always returns true for a validated v1alpha1 document.
    #[must_use]
