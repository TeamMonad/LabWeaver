//! Strict, bounded client and lifecycle for one-shot Resource task reservations.
//!
//! The Resource service owns approval, capacity, leases, and usage records. A task owner service
//! only submits a typed request and observes the authoritative response. This module keeps the
//! HTTP boundary deliberately small: HTTPS with an explicit CA, no redirects, bounded request and
//! response bodies, and an audience-scoped service token for every call. The lifecycle wrapper
//! turns that client into the durable create -> claim -> acknowledge -> release protocol shared by
//! every one-shot task producer.
...
use contracts::{
    ActorId, CourseId, ProjectId,
    resource::{CapacityClaimState, ResourceLeaseState, ResourceRequestState, WorkloadResources},
};
...
pub enum TaskResourceError {
    #[error("LW_TASK_RESOURCE_REQUEST_INVALID")]
    RequestInvalid,
    #[error("LW_TASK_RESOURCE_RESPONSE_INVALID")]
    ResponseInvalid,
    #[error("LW_TASK_RESOURCE_IDENTITY_MISMATCH")]
    IdentityMismatch,
    #[error("LW_TASK_RESOURCE_TERMINAL")]
    ResourceTerminal,
    #[error("LW_TASK_RESOURCE_APPROVAL_TIMEOUT")]
    ResourceApprovalTimeout,
    #[error("LW_TASK_RESOURCE_CANCELLED")]
    Cancelled,
    #[error("LW_TASK_RESOURCE_CREATE_UNCERTAIN")]
    CreateUncertain,
    #[error(transparent)]
...
pub enum ResourceClientError {
    #[error("LW_TASK_RESOURCE_HTTP_CONFIG_INVALID")]
    Configuration,
    #[error("LW_TASK_RESOURCE_HTTP_TOKEN_FAILED")]
    Token(#[source] ServiceTokenClientError),
    #[error("LW_TASK_RESOURCE_HTTP_TRANSPORT_FAILED")]
    Transport,
    #[error("LW_TASK_RESOURCE_HTTP_REQUEST_INVALID")]
    RequestInvalid,
    #[error("LW_TASK_RESOURCE_HTTP_REQUEST_TOO_LARGE")]
    RequestTooLarge,
    #[error("LW_TASK_RESOURCE_HTTP_RESPONSE_TOO_LARGE")]
    ResponseTooLarge,
    #[error("LW_TASK_RESOURCE_HTTP_RESPONSE_INVALID")]
    ResponseInvalid,
    #[error("LW_TASK_RESOURCE_HTTP_REQUEST_MISSING")]
    RequestMissing,
    #[error("LW_TASK_RESOURCE_HTTP_TASK_MISSING")]
    TaskResourceMissing,
    #[error("LW_TASK_RESOURCE_HTTP_DENIED")]
    Denied,
    #[error("LW_TASK_RESOURCE_HTTP_CONFLICT")]
    Conflict,
    #[error("LW_TASK_RESOURCE_HTTP_REJECTED")]
    Rejected,
    #[error("LW_TASK_RESOURCE_HTTP_UNAVAILABLE")]
    Unavailable,
}
