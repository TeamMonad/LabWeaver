//! Shared Kubernetes Job execution mechanics for one-shot workloads.
//!
//! This module owns the exact REST mutations, ownership verification, observation, and
//! deterministic cleanup that every one-shot execution backend shares. Role-specific document
//! rendering and evidence receipt parsing stay in the role executor; this module never interprets
//! a score, a probe fact, or an Agent result.
#![allow(
    missing_docs,
...
pub fn failed_container_diagnostic<'a>(
    terminated: &'a Value,
...
pub fn verify_cleanup_owned(current: &Value, expected: &Value) -> Result<(), KubernetesJobError> {
