    ...
    pub(crate) async fn build(
        ...
        Ok(match key_source_type {
            ...
            KeySourceType::Jwks(url) => {
                ...
                let key_store_manager = KeyStoreManager::new(
                    jwks_url,
            ...
            KeySourceType::Discovery(issuer_url) => {
                ...
                let jwks_url = Url::parse(&oidc::discover_jwks(issuer_url.as_str(), Some(http_client.clone())).await?)
                    .map_err(|e| InitError::JwksUrlError(e.to_string()))?;

                let key_store_manager =
                    KeyStoreManager::new(jwks_url, refresh.unwrap_or_default(), http_client);
                Authorizer {
                    key_source: KeySource::KeyStoreSource(key_store_manager),
                    claims_checker,
                    validation,
