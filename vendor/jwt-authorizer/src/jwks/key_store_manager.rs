...
impl KeyStoreManager {
    pub(crate) fn new(key_url: Url, refresh: Refresh, http_client: Client) -> KeyStoreManager {
        KeyStoreManager {
            key_url,
            http_client,
            refresh,
            ...
            RefreshStrategy::KeyNotFound => {
                if let Some(ref kid) = header.kid {
                    ...
                    if let Some(jwk) = jwk_opt {
                        jwk
                    } else if ks_gard.can_refresh_on_miss(self.refresh.retry_interval) {
                        ks_gard
                            .refresh(&self.http_client, &self.key_url, &[("kid", kid)])
                            .await?;
...
impl KeyStore {
    fn can_refresh_on_miss(&self, minimal_retry: Duration) -> bool {
        if let Some(fail_time) = self.fail_time {
            fail_time.elapsed() > minimal_retry
        } else {
            self.load_time
                .is_none_or(|load_time| load_time.elapsed() > minimal_retry)
        }
    }
    ...
    async fn refresh(
        ...
        metrics::counter!("labweaver_auth_jwks_refreshes", "result" => "attempt").increment(1);
        http_client
            .get(key_url.as_ref())
            .query(qparam)
            .send()
            ...
            .map_err(|e| {
                ...
                metrics::counter!("labweaver_auth_jwks_refreshes", "result" => "transport_error")
                    .increment(1);
            ...
            .map(|jwks| {
                ...
                if keys.is_empty() {
                    metrics::counter!("labweaver_auth_jwks_refreshes", "result" => "empty")
                        .increment(1);
                    Err(AuthError::JwksRefreshError("No valid keys in the Jwk Set!".to_owned()))
                } else {
                    self.keys = keys.into();
                    self.fail_time = None;
                    metrics::counter!("labweaver_auth_jwks_refreshes", "result" => "success")
                        .increment(1);
                    Ok(())
                }
            })
            .map_err(|e| {
                self.fail_time = Some(Instant::now());
                metrics::counter!("labweaver_auth_jwks_refreshes", "result" => "invalid")
                    .increment(1);
                AuthError::JwksRefreshError(e.to_string())
            })?
    }
