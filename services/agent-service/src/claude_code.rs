...
Use the exact JSON property spelling from the schema and never add unknown properties. In particular, outer EnvironmentSpec, resources, entries, security, ArtifactRef, retention, and the optional terminal object properties are camelCase, but runtime variant properties are exactly provider_binding, build_context, base_image_digest, service_port, terminal for container and provider_binding, base_disk, storage_class_binding, ssh_port for virtual_machine. Network is a tagged object whose mode is allow_all, deny_all, or restricted; restricted alone has policy_binding. Runtime kind is container or virtual_machine. Container security requires rootFilesystemPolicy read_only_required. A terminal may be emitted only when the verified materials explicitly approve it; it requires an absolute executable, a bounded direct-exec args array, and workingDirectory exactly /workspace. Never infer a shell or add a fallback executable. A virtual_machine requires mutable_required, an ssh entry on port 22, and must never use allow_all. All resource sizes and ports must be non-zero, entries must be non-empty with unique names, digests must be sha256 followed by 64 lowercase hexadecimal characters, identifiers must be UUID strings, and retainUntil must be an RFC 3339 timestamp.
