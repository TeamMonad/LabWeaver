...
CREATE TABLE frozen_submissions (
    frozen_submission_id uuid PRIMARY KEY, project_id uuid NOT NULL, course_id uuid, environment_id uuid NOT NULL,
    ...
    frozen_at timestamptz NOT NULL, CHECK (jsonb_typeof(contract) = 'object')
);
...
CREATE UNIQUE INDEX frozen_submissions_project_id_idempotency_key_idx
    ON frozen_submissions (project_id, idempotency_key)
