...
class ResourceAcceptanceProfileTests(unittest.TestCase):
    ...
    def test_bootstrap_owns_a_bounded_postgres_tunnel(self) -> None:
        ...
        self.assertIn("system_tmpdirs = /tmp", ansible_config)
    ...
    def test_browser_authentication_is_a_private_ansible_boundary(self) -> None:
        ...
        self.assertIn("--identity-seed", tasks)
        self.assertIn("follow: true", tasks)
        self.assertIn("Reconcile reviewed Keycloak replay credentials", tasks)
        self.assertIn("Build staged Resource replay Keycloak truststore", tasks)
        self.assertIn("Atomically publish Resource replay Keycloak truststore", tasks)
        self.assertIn("resource_replay_auth_keycloak_truststore_staged", defaults)
        self.assertIn("--token", tasks)
        self.assertIn("LW_RESOURCE_REPLAY_AUTH_KEYCLOAK_USERS_INVALID", tasks)
