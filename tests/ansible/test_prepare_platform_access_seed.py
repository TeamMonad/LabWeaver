...
SCRIPT = ROOT / "tools/prepare_platform_access_seed.py"
ADOPT_TEMPLATE = (
    ROOT
    / "deploy"
    / "ansible"
...
SPEC = importlib.util.spec_from_file_location("prepare_platform_access_seed", SCRIPT)
...
def test_seed_rejects_missing_role_and_public_output(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert str(error) == "LW_PLATFORM_ACCESS_SEED_USER_INVALID"
    else:
        raise AssertionError("missing role must be rejected")
    try:
        MODULE.private_output(tmp_path / "seed.json")
    except MODULE.AccessSeedError as error:
        assert str(error) == "LW_PLATFORM_ACCESS_SEED_PRIVATE_PATH_REQUIRED"
    else:
...
def test_seed_rejects_non_v7_course_identifier() -> None:
    ...
    try:
        ...
        assert str(error) == "LW_PLATFORM_ACCESS_SEED_COURSE_INVALID"
    else:
...
def test_adoption_only_reconciles_unowned_legacy_v5_actor_ids() -> None:
    ...
    assert "PLATFORM_LEGACY_ACTOR_HAS_OWNED_DATA" in template
