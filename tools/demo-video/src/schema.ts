...
let validators: Promise<{receipt: ValidateFunction; manifest: ValidateFunction; releaseGate: ValidateFunction}> | undefined;
...
export async function getValidators(root: string) {
  validators ??= (async () => {
    ...
    addFormats(ajv);
    const [receiptSchema, manifestSchema, releaseGateSchema] = await Promise.all([
    ...
    return {receipt: ajv.compile(receiptSchema), manifest: ajv.compile(manifestSchema), releaseGate: ajv.compile(releaseGateSchema)};
  })();
  return validators;
}
