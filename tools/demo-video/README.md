...
The rendered deliverable is 1920×1080, 60 fps, H.264, silent, and 4:30 long. Contextual Chinese explanation cards cover the platform overview and every product step; bilingual subtitles remain external as `captions/zh-CN.srt` and `captions/en-US.srt`. Rendering requires a hardware H.264 encoder, uses a fixed 12 Mbit/s video bitrate, and uses four Chromium frame workers for the deadline-bound local render. Remotion is configured with `hardwareAcceleration: required`, so an unavailable or incompatible encoder is a blocking error rather than a silent software fallback. Each source clip plays exactly once after the one-second navigation trim, then its captured final screenshot remains as the background while distinct explanation beats continue. Looping is forbidden; clips too short for the trim fail with `LW_DEMO_VIDEO_CLIP_TOO_SHORT`, and clips longer than their allocation fail with `LW_DEMO_VIDEO_CLIP_TIMELINE_OVERFLOW`. The complete renderer profile participates in every scene chunk's input hash, so the former looping chunks cannot be reused. Repeating `verify` reruns every seek without rewriting the first successful playback screenshot, keeping the verified manifest byte-stable.
