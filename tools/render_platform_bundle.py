...
API_VERSION = "deploy.labweaver.io/platform-bundle-manifest/v1"
...
def _load_manifest(path: Path) -> dict[str, Any]:
    try:
        ...
        raise BundleError("LW_PLATFORM_BUNDLE_MANIFEST_INVALID") from error
    if not isinstance(value, dict) or set(value) != {
        ...
        raise BundleError("LW_PLATFORM_BUNDLE_MANIFEST_INVALID")
...
def _read_exact_files(directory: Path, keys: list[str], binary: bool) -> dict[str, str]:
    if not directory.is_dir() or directory.is_symlink():
        raise BundleError("LW_PLATFORM_BUNDLE_INPUT_INVALID")
    entries = {entry.name for entry in directory.iterdir()}
    if entries != set(keys):
        raise BundleError("LW_PLATFORM_BUNDLE_INPUT_INCOMPLETE")
    result: dict[str, str] = {}
    for key in sorted(keys):
        ...
        if binary:
            ...
            try:
                ...
                raise BundleError("LW_PLATFORM_BUNDLE_INPUT_INVALID") from error
    return result
...
def _validate_jetstream_ack_permission(secret_name: str, directory: Path) -> None:
    ...
    try:
        ...
        raise BundleError("LW_PLATFORM_NATS_CREDENTIALS_INVALID") from error
    if not isinstance(publish_allow, list) or "$JS.ACK.>" not in publish_allow:
        raise BundleError("LW_PLATFORM_NATS_ACK_PERMISSION_REQUIRED")
...
def _require_private_path(path: Path) -> None:
    if ".private" not in path.parts:
        raise BundleError("LW_PLATFORM_BUNDLE_PRIVATE_PATH_REQUIRED")


def main() -> int:
    root = Path(__file__).resolve().parents[1]
    parser = argparse.ArgumentParser()
    parser.add_argument("--manifest", type=Path, default=root / "deploy/config/platform-bundle-manifest.json")
    ...
    try:
        ...
        diagnostic = str(error) if isinstance(error, BundleError) else "LW_PLATFORM_BUNDLE_WRITE_FAILED"
