...
a subset of `llmReadable`. The current v1alpha1 shape binds one Collector, so `system_facts`
...
Collector include/exclude values in this v1alpha1 contract are literal normalized paths. An exclude
...
- The v1alpha1 Ansible module allowlist does not prove HTTP behavior for the Linux golden path.
