...
| Resource request and Lease authority (#142) | private UUIDv7 course bootstrap, Work-only AgentRun, independent candidate approval/release, owner-scoped request list/get/create, approve/resize-approve, cancel, reject, retry, Lease renew/revoke; revision/idempotency fences; exact Environment Lease sync; Access-first expiry cleanup; capacity absence readback | profile/Access membership mismatch, experiment/missing Work candidate, missing/invalid caller, cross-owner read/cancel/retry, duplicate/conflicting idempotency, stale revision, expired approval, zero/unsupported GPU, Lease renewal/revoke replay, Environment cleanup failure, capacity residue | connected PostgreSQL/NATS/Kubernetes quota-shell adoption, approved Work handoff, renewal, revoke/expiry and residue-free release on one deployment identity, bound into Release Gate v3 |
| Container xterm console (#131/#126) | AccessGrant discovery/recovery, exact 30-second issuance, atomic redemption, Access→Environment→executor mTLS, unique Ready runtime Pod, fixed `runtime` PTY, binary I/O, bounded resize/fullscreen and manual reconnect | Origin/CSRF/ETag/idempotency, Work/Experiment Lease rules, consumed handoff, revision drift, SAN/subprotocol failure, missing/ambiguous Pod, revoke, short Grant expiry, stop, delete and label-fenced NATS control-channel loss | connected Playwright matrix, v1 evidence validator and Release Gate v3 check are implemented and locally testable; real cluster evidence remains blocked until #126 execution prerequisites clear |
| KubeVirt noVNC console (#124/#126) | same capability/session foundation with runtime-tagged Environment binding, mTLS bridge, fixed-VMI `/vnc` executor and accessible real-RFB connected state | no guest password/public endpoint; stale revision/Lease/VMI identity, stream limit, consumed handoff, revoke, short Grant expiry, stop, delete, control-channel loss and policy recovery | connected Playwright matrix, v1 evidence validator and Release Gate v3 check are implemented and locally testable; real browser/VMI evidence remains blocked until #126 execution prerequisites clear |
...
| Web and Release Gate | teacher/student Playwright, two six-case console matrices, demo replay, v3 report generation | fixed sleep, Fixture/Mock bundle, missing console check/case, stale/cross-Run identity, tampering, path escape, missing cleanup, sensitive fields or skipped gate | one v3 report parses and references all thirteen same-identity connected checks |
