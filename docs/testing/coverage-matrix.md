...
| Playwright role-project configuration | four role projects, A-approved `researcher` alias under `student`, `platform-admin` mapping to Web `admin`, retained failure artifacts, and E1 report | invalid project set, fixed sleeps, missing/invalid base URL, missing authentication setup, missing storage state, or zero runtime tests fail closed | E1 static configuration and contract tests; runtime E2E not executed | approved auth setup, role storage states, real browser paths, authorization isolation, and E3/E4 evidence |
