...
| Container xterm console (#131) | AccessGrant discovery, exact 30-second issuance, atomic redemption, Access→Environment→executor mTLS, unique Ready runtime Pod, fixed `runtime` PTY, binary I/O and bounded resize | Origin/CSRF/ETag/idempotency, Work/Experiment Lease rules, concurrent/expired/consumed handoff, revision drift, SAN/subprotocol failure, missing/ambiguous Pod, frame/output limit, revoke/logout/expiry/stop and overdue | implemented and locally verified only; real cluster PTY and connected control-loss evidence belong to #126 |
| KubeVirt noVNC console (#124) | same capability/session foundation with runtime-tagged Environment binding, mTLS bridge and dedicated fixed-VMI `/vnc` executor | no guest password/public endpoint; local stale revision/Lease/VMI identity, stream limit, disconnect, RBAC and NetworkPolicy checks | implemented and locally verified; real browser-to-VMI, connected revoke/expiry/stop/delete and Release Gate remain #126 scope |
