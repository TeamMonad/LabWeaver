...
| --- | --- | --- | --- |
| Teacher authoring | Keycloak login, material upload, AgentRun, two candidates, independent approval | invalid hash/schema, refusal, cancellation, cross-course mutation, unapproved release | real browser and backend share one identity |
...
| Deployment | clean reset, baseline migrations, ordered deploy, second idempotent deploy | non-LabWeaver Kyverno dependency, missing locator, invalid digest, readiness failure, cleanup residue | sanitized manifest and readback bind the deployed identity |
