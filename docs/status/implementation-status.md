...
| Sprint 2 Helm deployment | A/D | implemented; older head deployed | ten-workload profile, seven-image package contract, per-process config/Secret mounts, health probes, independent Container/KubeVirt/Evaluation RBAC and non-destructive adoption; every Pod template binds the exact private configuration-bundle SHA-256 | cp1, kube-apiserver and retained KubeVirt/CDI are responding again; the final-head package, private config validation, two application reconciles and runtime verification remain pending |
...
| Sprint 2 retained-infrastructure adoption | A/D | older head application adoption verified; final-head reconcile pending | `sprint2-buildkit` adopted retained BuildKit and exact mTLS Buildx; `sprint2-application` deployed an older head twice without delete operations and retained an atomic rollback diagnostic; the Control NATS credential was forward-rotated without resetting retained infrastructure | cp1 and kube-apiserver recovered on 2026-07-21; the final source identity must now be packaged and reconciled twice before this row can be verified |
