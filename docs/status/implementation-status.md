...
| Keycloak identity foundation for #61 | A, requires B security review and D Verify | deployed on the adopted cluster; automated reconciliation pending | WSL direct run `infra-identity-direct-20260715-02`: fixed-digest PostgreSQL/Keycloak Ready, namespace CA/TLS Ready, Gateway `10.20.0.222` Programmed, exact issuer plus workload `aud`/`azp`/`preferred_username` verified, DNS probe passed | E3 identity baseline | self-signed private-lab CA is not a public PKI; the approved `identity-foundation --infra` replay, PostgreSQL backup/recovery and D Verify remain required |
