...
| Environment lifecycle v1 | A | typed and semantically validated, pending human contract and security review | ADR 0004, EnvironmentLifecycle v1 contract and transition tests | E1 | no persistence, Lease runtime, Provider, CRD, KubeVirt or cleanup runtime evidence |
...
| Private Sigstore trust plane (#61) | D, requires A+B security review | C0 authority and Trillian foundation deployed; complete trust plane pending | C0 run `2026-07-15`: 3-key/2-of-3 TUF root verified, independent Fulcio/Rekor/CT/TLS material provisioned, immutable TUF and Tree authority ConfigMaps, Trillian MySQL plus 2 LogServer/2 LogSigner Ready, distinct Rekor/CT Tree IDs, static authoritative TUF root hash verified; locked Linux syntax/lint and restricted Pod Security full render passed | E3 for C0/Trillian foundation; E1/E2 for remaining deployment path | full Fulcio/Rekor/CT reconciliation, Gateway/DNS, keyless signing, SCT/Rekor proof, backup/restore/rotation/DR, Kyverno and Release Gate runtime evidence remain required |
| Keycloak identity foundation for #61 | A, requires B security review and D Verify | deployed on the adopted cluster; automated reconciliation pending | WSL direct runs `infra-identity-direct-20260715-02` and `-03`: fixed-digest PostgreSQL/Keycloak Ready, namespace CA/TLS Ready, Gateway `10.20.0.222` Programmed, exact issuer plus workload `aud`/`azp`/`preferred_username` verified, DNS probe and second client reconciliation passed | E3 identity baseline | self-signed private-lab CA is not a public PKI; the approved `identity-foundation --infra` replay, PostgreSQL backup/recovery and D Verify remain required |
