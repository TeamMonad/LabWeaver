...
| Approved Container supply chain and runtime (#52) | B implementation; requires A+B security review and D Verify | local production wiring and deterministic tests implemented in current worktree; connected E3 blocked | v2 full-payload build/release events and generated schemas; approval-to-BuildRequest Control Outbox plus exact persisted candidate/approval/artifact projection; durable Agent Inbox/fenced heartbeat worker/live cancellation/atomic artifact-policy/project-evidence-Outbox; exact per-course private Harbor Project/quota/robot identity and restricted BuildKit/Trivy/Private Sigstore provider protocol; Critical block, High warning, issuer/subject/Rekor/SCT/build-window and digest gates; authoritative Control artifact readback; immutable Environment release projection; deterministic least-privilege namespace/finalizer/Quota/LimitRange/ServiceAccount/imagePullSecret/PVC/Deployment/ClusterIP Service/NetworkPolicy/unique protected HTTPRoute; eight pipeline and four Provider tests; PostgreSQL 17 Control command/release identity, Agent lease/cancel/cleanup/successful artifact+project evidence/single terminal-Outbox and complete Migration catalog tests passed 2026-07-16 | E1 deterministic behavior and process wiring; E2 PostgreSQL Migration, fencing, cancellation and transaction evidence | deployment-owned BuildKit/Harbor/Trivy/Private Sigstore and Kubernetes executors are not implemented or replayed in this worktree; no connected same-build E3 proof, human approval or D Verify |
