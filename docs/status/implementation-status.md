...
The current source identity is `3a9ac6c1a31040570a303b80309a23ce5ea635e1` on
`release/sprint2`. Non-destructive application adoption completed as run
`sprint2-application-3a9ac6c1-2315`, Helm revision `162`, and cluster UID
...
| --- | --- | --- |
| Container environment reconcile | blocked on current release | The current Web/Keycloak path reaches the real release list, but the oldest row is expired and the newest row reaches the Environment worker; its retained Trivy evidence is stale and the worker fails closed with `LW_ENVIRONMENT_RELEASE_EVIDENCE_EXPIRED`. |
...
| Release Gate / Sprint 2 | blocked | The 3a9 adoption is healthy and KubeVirt is intentionally skipped, but a current same-identity Container create/access replay is blocked by stale retained image evidence; no machine-readable passing Release Gate exists. |
...
| BuildKit/Harbor/Trivy supply chain | B | platform package verified; course replay blocked | fixed-command `build-executor`, persistent fence and negative tests; 3a9 produced seven digest-bound Harbor images and passed connected `package-validate` against the locked Trivy database | newly uploaded course context fails closed with `LW_AGENT_BUILD_REJECTED`; exact object/validation cause is currently redacted |
...
| Sprint 2 Helm deployment | A/D | implemented; 3a9 adopted non-destructively | source `3a9ac6c1a31040570a303b80309a23ce5ea635e1` passed connected package validation and non-destructive application adoption run `sprint2-application-3a9ac6c1-2315`; live readback showed all ten workloads Ready and seven Harbor digest images; retained infrastructure was not deleted or rebuilt | current-head Container replay, Gateway protocol replay, KubeVirt replay and Release Gate remain pending |
...
| Sprint 2 retained-infrastructure adoption | A/D | 3a9 verified | `sprint2-buildkit` retained BuildKit and exact mTLS Buildx; application run `sprint2-application-3a9ac6c1-2315` retained all named infrastructure, reported `destructive_reset=false`, and reconciled the ten-workload identity | retained infrastructure is not dual-runtime E3 evidence |
