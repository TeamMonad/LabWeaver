...
| Private Sigstore trust plane (#61) | D, requires A+B security review | repository lifecycle implemented; real environment blocked | version/digest lock, generated Rust contracts, negative tests, fixed action-to-playbook mapping, backup-first lifecycle orchestration, run-scoped cleanup and Helm render gate | E1 after local/CI gates | local router host key is not pinned; no E3 deploy/idempotency, signing, SCT/Rekor proof, restore/rotation/DR, Kyverno or Release Gate runtime evidence |
| Keycloak identity foundation for #61 | A, requires B security review and D Verify | deployed on the adopted cluster; automated reconciliation pending | WSL direct runs `infra-identity-direct-20260715-02` and `-03` recorded fixed-digest PostgreSQL/Keycloak readiness, CA/TLS readiness, Gateway programming, exact issuer/workload claims, DNS probe and second reconciliation. Read-only verifier observation at `0a1309d` separately confirms API, rollout, Gateway and Certificate readiness without Secret access or mutation; see `docs/testing/evidence/identity-foundation-verifier-20260715.md`. | E3 identity baseline | self-signed private-lab CA is not a public PKI; the approved `identity-foundation --infra` replay, PostgreSQL backup/recovery and full D Verify remain required |
