...
Issue #131 additionally has a connected, mixed-source demonstration slice, not
a release deployment. Environment/container-executor source
`96d9ca3dffc93e28ea61583a9cd7ecc54d578973` was pushed as
`sha256:5cd9d553e3d39ed6ccc64abac2a8aed75160fa9e7b57dc98a2ed692ac401e679`;
the other application images predate that source because the blocked rootless
builder cannot produce a same-source seven-image package. A real browser login,
AccessGrant, xterm PTY write under `/workspace`, disconnect/reconnect,
full-screen transition, revoke and denied post-revoke reconnect passed. A
second identical Helm reconciliation retained every selected Pod UID. This
proves the interactive Container demonstration only. It does not satisfy
same-identity packaging, Ansible Verify, KubeVirt, Release Gate or D acceptance.
...
| AccessGrant and session authorization | A | implemented; terminal revoke connected in mixed-source demo | fixed local OpenSSH account, post-auth alias redemption, one-time token and session contracts; HTTP(S) grants use same-origin `/connect` paths; Issue #131 adds an explicit Container-only terminal route with database-backed global admission, five-second authorization heartbeat and no transcript; the connected demo closed all four recent terminal metadata sessions and denied a reconnect after revoke | HTTP/SSH expiry, cross-course and crash-recovery remain pending; the connected terminal result is not same-source release evidence |
| Container browser terminal | A/B/C/D | connected demonstration verified; formal deployment blocked | optional approved `TerminalSpec`, `browser_terminal` capability, Access→Environment→container-executor mTLS WebSocket chain, explicit kube 0.99.0 PTY exec, executor-only `pods get/list/watch` and `pods/exec get/create`, xterm.js 6.0.0 UI; Contracts/Auth/Access/Environment, PostgreSQL migrations, Web gates, config/Helm render and focused RBAC tests pass; source `96d9ca3d` completed real mTLS executor and browser login/write/reconnect/fullscreen/revoke replay | rootless BuildKit cannot produce the same-identity seven-image package; stop/start, freeze, cross-course, delete cleanup, D Verify and Release Gate remain open; the local cluster must be recycled before security/release evidence because infrastructure Secret material entered transient diagnostic output |
