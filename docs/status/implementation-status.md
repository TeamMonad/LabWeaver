...
| Sprint 2 retained-infrastructure adoption | A/D | implemented locally | `sprint2-application` validates the seven-image package, adopts empty or exact-ledger schemas, creates only missing streams/bucket/realm, reconciles reviewed configuration and deploys twice without delete operations | complete private application bundle and connected execution remain pending |
