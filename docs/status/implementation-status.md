...
| Platform image trusted supply chain (#62) | A implementation; requires A+B security review and D Verify | local implementation complete; connected path blocked | seven pinned multi-stage images, strict build context, version lock, atomic `xtask` package/validate/deploy/rollback path, versioned result schemas, digest-only Helm/Kyverno policy, PR workflow, Rust negative tests and Linux Helm/Kyverno render fixtures | E1 plus local E2-static structure; no connected E3 | Docker/BuildKit image execution and controlled Harbor/Sigstore/admission replay are not available in the current local environment; #61 post-merge identity replay, production Config/Secret locators and read-only cluster baseline remain mandatory blockers |
