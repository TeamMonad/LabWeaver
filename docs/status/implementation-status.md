...
| Claude Code Agent runtime boundary | B | implemented in current worktree, pending A+B review and D Verify | `ClaudeCodeBindingV1`, generated Schema, ProblemPackage hash/classification gate, pinned shell-free worker, exact-Schema JSON prompt, strict local typed/semantic validation, terminal `stream-json` parsing, immutable budgets, cancellation, hash-only audit, dual-track partial-success tests, and Agent-owned idempotency/checkpoint/Outbox implementation; ephemeral PostgreSQL 17 integration passed on 2026-07-14; real local Environment generation passed with CLI `2.1.209` in 9.31 seconds at 34,755 microusd | E1 runtime boundary plus live local provider evidence; E2 persistence | Evaluation and dual-track real-provider calls are not yet verified; provider environment came from login-shell exports and is not deployment evidence; no pinned worker image/config, HTTP handler, Control candidate projection, JetStream publisher, object-store checkpoint reference, isolated Kubernetes Job or E3 credential path |
