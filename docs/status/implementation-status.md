...
| Contracts, OpenAPI and Web SDK | A/C | implemented locally | one ADR 0011 v1 Rust source with generated JSON Schema, Public/Internal OpenAPI and Web SDK; all Draft-head contract/render and Web drift gates pass | connected clients and deployed schema identity remain pending |
...
| Claude Code Agent runtime | B | implemented; older head deployed | bounded process runtime, leases, cancellation, candidate validation and explicit ECNU Anthropic-compatible endpoint/Secret binding; the previous deployed head completed both real ECNU candidate generations, exposing a live-lease replay defect and missing Control quarantine permission that are fixed and regression-tested at the current head | current-head deployment and a fresh dual-candidate replay are pending; the previous run is diagnostic evidence, not a passing gate |
| BuildKit/Harbor/Trivy supply chain | B | implemented; current head packaged | fixed-command `build-executor`, persistent fence and negative tests; source `d85855e9a236ba605411921b8575d1f785acfde5` produced seven digest-bound Harbor images, passed the Trivy policy and passed connected `package-validate` | application rollout and a real course build/publication remain pending; platform package evidence alone cannot satisfy the runtime gate |
...
| Sprint 2 Helm deployment | A/D | implemented; older head deployed | ten-workload profile, seven-image package contract, per-process config/Secret mounts, health probes, independent Container/KubeVirt/Evaluation RBAC and non-destructive adoption; every Pod template now binds the exact private configuration-bundle SHA-256; current-head package and private configuration input passed identity validation | current-head rollout and RBAC/runtime verification are pending because the control-plane management services are not responding |
...
| Sprint 2 retained-infrastructure adoption | A/D | older head application adoption verified; current head blocked before mutation | `sprint2-buildkit` adopted retained BuildKit and exact mTLS Buildx; `sprint2-application` deployed an older head twice without delete operations and retained an atomic rollback diagnostic; current-head packaging and connected validation pass, and the Control NATS credential was forward-rotated and validated without resetting retained infrastructure | router, worker and NFS preflight passes, but `k8s-cp1` currently accepts ICMP/TCP while sshd and kube-apiserver do not return application-layer responses; the current-head reconcile therefore stops before application changes |
