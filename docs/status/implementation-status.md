...
| Keycloak identity foundation for #61 | A, requires B security review and D Verify | implemented in current worktree, runtime verification pending | fixed-digest Keycloak/PostgreSQL, namespace-scoped private CA, exact internal issuer, workload-only client and allowlisted `identity-foundation --infra` entry | E1 pending gates | self-signed private-lab CA is not a public PKI; router replay, token-claim proof, backup and recovery evidence remain required |
