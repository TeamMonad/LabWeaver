...
| BuildKit/Harbor/Trivy supply chain | B | implemented; prior head packaged | fixed-command `build-executor`, persistent fence and negative tests; source `d85855e9a236ba605411921b8575d1f785acfde5` produced seven digest-bound Harbor images, passed the Trivy policy and passed connected `package-validate` | the final source identity must be rebuilt; application rollout and a real course build/publication remain pending; platform package evidence alone cannot satisfy the runtime gate |
...
| Sprint 2 Helm deployment | A/D | implemented; prior Draft head deployed | ten-workload profile, seven-image package contract, per-process config/Secret mounts, health probes, independent Container/KubeVirt/Evaluation RBAC and non-destructive adoption; source `e558ff27` rolled out all ten workloads twice with exact digest-only images, then correctly blocked because Cilium did not reconcile the attempted TCPRoute; the current source removes that unsupported route and uses the existing reviewed shared-VIP LoadBalancer contract | the current source identity must be packaged and reconciled twice; real runtime verification remains pending |
...
| Sprint 2 retained-infrastructure adoption | A/D | prior Draft head application rollout verified; final-head reconcile pending | `sprint2-buildkit` adopted retained BuildKit and exact mTLS Buildx; `sprint2-application` rolled out source `e558ff27` twice without delete operations and retained the blocking SSH exposure diagnostic; the Control NATS credential was forward-rotated without resetting retained infrastructure | cp1 and kube-apiserver recovered on 2026-07-21; the final source identity and shared-VIP SSH Service must now be packaged and reconciled twice before this row can be verified |
