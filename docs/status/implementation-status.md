...
| Sprint 2 retained-infrastructure adoption | A/D | BuildKit adoption verified; application adoption blocked | `sprint2-buildkit` adopted the retained BuildKit, loopback tunnel and exact mTLS Buildx binding; `sprint2-application` validates the seven-image package, adopts empty or exact-ledger schemas, creates only missing streams/bucket/realm, reconciles reviewed configuration and deploys twice without delete operations | the private Agent credential is missing; application rollout and connected runtime verification have not run |
