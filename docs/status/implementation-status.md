...
| Claude Code Agent runtime | B | implemented locally | bounded process runtime, leases, cancellation, candidate validation and explicit ECNU Anthropic-compatible endpoint/Secret binding tests | the authorized `ECNU_API_KEY` is not yet visible in the private deployment input; dual-candidate live replay is pending |
| BuildKit/Harbor/Trivy supply chain | B | verified through connected package validation | fixed-command `build-executor`, persistent fence and negative tests; the Draft PR head has a seven-image BuildKit/Harbor/Trivy package with zero Critical findings and connected digest readback | this is package evidence, not application rollout or runtime E3 |
...
| Web teacher/student journeys | C | verified in SDK and fixture modes | component/SDK tests and the Linux teacher/student Playwright fixture matrix pass at the Draft PR head; retired Evaluation/Resource navigation and signing evidence are absent | real Keycloak and same-build backend Playwright remain pending |
...
| Sprint 2 retained-infrastructure adoption | A/D | BuildKit adoption verified; application adoption blocked | `sprint2-buildkit` adopted the retained BuildKit, loopback tunnel and exact mTLS Buildx binding; `sprint2-application` validates the seven-image package, adopts empty or exact-ledger schemas, creates only missing streams/bucket/realm, reconciles reviewed configuration and deploys twice without delete operations | the ECNU token has been authorized but is not visible in the private deployment input; application rollout and connected runtime verification have not run |
